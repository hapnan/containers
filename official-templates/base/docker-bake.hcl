...
target "cpu-ubuntu2204" {
  ...
  tags = [
    "hapnan/base:${RELEASE_VERSION}${RELEASE_SUFFIX}-ubuntu2204",
  ]
  args = {
...
target "cpu-ubuntu2404" {
  inherits = ["common-base"]
  tags = [
    "hapnan/base:${RELEASE_VERSION}${RELEASE_SUFFIX}-ubuntu2404",
  ]
  args = {
...
target "cuda-matrix" {
  ...
  tags = [
    "hapnan/base:${RELEASE_VERSION}${RELEASE_SUFFIX}-cuda${combo.cuda_code}-${combo.ubuntu_name}",
  ]
